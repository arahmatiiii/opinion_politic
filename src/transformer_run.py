"""
transformer_run.py is written for run TRANSFORMER model
"""
...
from opinion_politic.utils.data_util import DataSet, initialize_weights_xavier_uniform_
...
from opinion_politic.methods.transformer_model import Encoder, Transformer
from opinion_politic.config.transformer_config import LOG_PATH, \
    TRAIN_DATA_PATH, TEST_DATA_PATH, VALID_DATA_PATH, \
    EMBEDDING_PATH, USE_POS, DEVICE, \
    N_EPOCHS, MODEL_PATH, USE_AUG, TEST_USE_AUG, \
    TEST_AUG_LOG_PATH, STEP_LR, OUTPUT_DIM, IS_TRANSFORMER, \
    HID_DIM, ENC_LAYERS, ENC_PF_DIM, ENC_HEADS, ENC_DROPOUT, MAX_LEN
...
class RunModel:
    ...
    def init_model(data_set, use_pos):
        ...
        """
        # create model
        enc = Encoder(vocab_size=data_set.num_vocab_dict["num_token"],
                      hid_dim=HID_DIM,
                      n_layers=ENC_LAYERS,
                      n_heads=ENC_HEADS,
                      pf_dim=ENC_PF_DIM,
        ...
        model = Transformer(hid_dim=HID_DIM, encoder=enc, output_size=OUTPUT_DIM,
                            device=DEVICE, src_pad_idx=data_set.pad_idx_dict["token_pad_idx"])

        # initializing model parameters
        model.apply(initialize_weights_xavier_uniform_)
