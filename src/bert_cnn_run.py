"""
bert_cnn_run.py is written for run Bert Cnn model
"""
...
from opinion_politic.utils.bert_data_util import DataSet, init_weights
...
from opinion_politic.methods.bert_cnn_model import BertCnn
from opinion_politic.config.bert_cnn_config import LOG_PATH, \
    TRAIN_DATA_PATH, TEST_DATA_PATH, VALID_DATA_PATH, \
    EMBEDDING_PATH, START_DROPOUT, \
    MIDDLE_DROPOUT, FINAL_DROPOUT, USE_POS, DEVICE, \
    N_EPOCHS, MODEL_PATH, N_FILTERS, FILTER_SIZES, \
    USE_AUG, TEST_USE_AUG, TEST_AUG_LOG_PATH, \
    STEP_LR, OUTPUT_DIM, bert_model
...
class RunModel:
    ...
    def init_model(data_set, use_pos):
        ...
        model = BertCnn(bert=bert_model, n_filters=N_FILTERS, filter_sizes=FILTER_SIZES,
                        start_dropout=START_DROPOUT, middle_dropout=MIDDLE_DROPOUT,
                        final_dropout=FINAL_DROPOUT, output_dim=OUTPUT_DIM)
