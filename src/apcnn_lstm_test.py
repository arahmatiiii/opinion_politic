"""
apcnn_lstm_test.py is written for test APCNN_LSTM model
"""
...
from opinion_politic.evaluation.apcnn_lstm_evaluation import PREDICT_SAMPLE_APCNN_LSTM
from opinion_politic.methods.apcnn_lstm_model import APCNN_LSTM
from opinion_politic.config.apcnn_lstm_config import HIDDEN_DIM, BIDIRECTIONAL, N_LAYERS,\
...
class TestModel:
    ...
    def init_model(self):
        ...
        model = APCNN_LSTM(vocab_size=len(self.text_field.vocab),
                           ...
                           pad_idx=self.text_field.vocab.stoi[self.text_field.pad_token], output_size=OUTPUT_DIM)
    ...
    def test(self, test_sentence):
        ...
        predict_sample = PREDICT_SAMPLE_APCNN_LSTM(model=model.to(DEVICE), tokenizer=self.tokenizer,
                                                text_field=self.text_field)
        results_tagged = predict_sample.predict_apcnn_lstm(test_sentence)
...
class TestUser:
    ...
    def __init__(self):
        ...
        self.test_apcnn_lstm_model = TestModel()
    ...
    def predict_user(self):
        ...
        for user_index, user in enumerate(self.evaluation_users.user):
            ...
            for i, item in enumerate(user_csv_file.tweet):
                if self.test_apcnn_lstm_model.test(item) == 1:
                    user_pos_count += 1
                else:
                    user_neg_count += 1
        ...
        all_results.to_csv(self.eval_user_path.replace('.csv', '_results_apcnn_lstm.csv'), index=False)

    def report_metric(self):
        evaluation_user_res = pandas.read_csv(self.eval_user_path.replace('.csv', '_results_apcnn_lstm.csv'))
