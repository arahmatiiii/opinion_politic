"""
cnn_test.py is written for test CNN model
"""
...
from opinion_politic.methods.cnn_model import CNN
from opinion_politic.evaluation.cnn_evaluation import PREDICT_SAMPLE_CNN
from opinion_politic.config.cnn_config import TEST_DATA_PATH,\
    START_DROPOUT, MIDDLE_DROPOUT, FINAL_DROPOUT,\
    MODEL_PATH, DEVICE, EVAL_USER_DATA_PATH, EVAL_USER_PATH, \
    TEXT_FIELD_PATH, EMBEDDING_DIM, N_FILTERS, FILTER_SIZES, EMBEDDING_PATH
...
class TestModel:
    ...
    def init_model(self):
        ...
        model = CNN(vocab_size=len(self.text_field.vocab),
    ...
    def test(self, test_sentence):
        ...
        predict_sample = PREDICT_SAMPLE_CNN(model=model.to(DEVICE), tokenizer=self.tokenizer,
                                             text_field=self.text_field)
        results_tagged = predict_sample.predict_cnn(test_sentence)
...
class TestUser:
    ...
    def predict_user(self):
        ...
        all_results.to_csv(self.eval_user_path.replace('.csv', '_results_cnn.csv'), index=False)

    def report_metric(self):
        evaluation_user_res = pandas.read_csv(self.eval_user_path.replace('.csv', '_results_cnn.csv'))
