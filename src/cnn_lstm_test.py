"""
cnn_lstm_test.py is written for test CNN  LSTM model
"""
...
from opinion_politic.evaluation.cnn_lstm_evaluation import PREDICT_SAMPLE_CNN_LSTM
from opinion_politic.methods.cnn_lstm_model import CNNLSTM
from opinion_politic.config.cnn_lstm_config import HIDDEN_DIM, BIDIRECTIONAL, N_LAYERS,\
    START_DROPOUT, MIDDLE_DROPOUT, FINAL_DROPOUT,\
    MODEL_PATH, DEVICE, EVAL_USER_DATA_PATH, EVAL_USER_PATH, \
    TEXT_FIELD_PATH, EMBEDDING_DIM, N_FILTERS, FILTER_SIZES
...
class TestModel:
    ...
    def init_model(self):
        ...
        model = CNNLSTM(vocab_size=len(self.text_field.vocab),
    ...
    def test(self, test_sentence):
        ...
        predict_sample = PREDICT_SAMPLE_CNN_LSTM(model=model.to(DEVICE), tokenizer=self.tokenizer,
                                                    text_field=self.text_field)
        results_tagged = predict_sample.predict_cnn_lstm(test_sentence)
    ...
    def predict_user(self):
        ...
        for user_index, user in enumerate(self.evaluation_users.user):
            ...
            for i, item in enumerate(user_csv_file.tweet):
                if self.test_cnn_model.test(item) == 1:
                    user_pos_count += 1
                else:
                    user_neg_count += 1
        ...
        all_results.to_csv(self.eval_user_path.replace('.csv', '_results_cnn_lstm.csv'), index=False)

    def report_metric(self):
        evaluation_user_res = pandas.read_csv(self.eval_user_path.replace('.csv', '_results_cnn_lstm.csv'))
